    ...
    bool clockSource, bool source)
{
    ...
    if (clockSource) {
        if (source) {
            ...
            msg = xferRtpTxAudio(context, context->rtpMsgTx[0], cd);
            if (msg) {
                sendMsg(sae, msg);
            }
            msg = xferVbanTxAudio(context, context->vbanMsgTx[0], cd);
            if (msg) {
                sendMsg(sae, msg);
            }
            ...
            msg = xferRtpRxAudio(context, context->rtpMsgRx[0], cd);
            if (msg) {
                sendMsg(sae, msg);
            }
            msg = xferVbanRxAudio(context, context->vbanMsgRx[0], cd);
            if (msg) {
                sendMsg(sae, msg);
            }
